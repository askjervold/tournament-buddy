...
function Match({ match }: MatchProps) {
  const [player1Wins, setP1Wins] = useState<string>(match.result.player1Wins?.toString() || '');
  const [player2Wins, setP2Wins] = useState<string>(match.result.player2Wins?.toString() || '');
  const [draws, setDraws] = useState<string>(match.result.draws?.toString() || '');
  ...
  const submitResult = () => {
    ...
    setRounds(prev => prev.map(r => {
      ...
      return { ...r, matches: r.matches.map(m => {
        return m.table === match.table ? { ...m, result, submitted: true } : m;
      }) }
    }));
  }
  ...
  return (
    <article>
      ...
      {match.submitted ?
        <p className="result">
          {match.result.player1Wins}-{match.result.player2Wins}-{match.result.draws}
        </p> :
        match.player2 && <section className="result-input">
          <input
            type="number"
            value={player1Wins}
